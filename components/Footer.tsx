...
export default function Footer() {
  return (
    <footer className="relative mt-24 overflow-hidden bg-ink-950 text-white">
      ...
      <div className="section relative grid gap-12 py-16 md:grid-cols-4">
        ...
        <div>
          <h4 className="mb-6 text-xs font-bold uppercase tracking-[0.25em] text-white">Legal</h4>
          <ul className="space-y-3">
            {legalLinks.map((l) => (
              <li key={l.label} className="flex items-center gap-3">
                <span className="h-1.5 w-1.5 rounded-full bg-brand-400" />
        ...
        <div>
          ...
          <p className="mt-5 text-white/70 text-sm">Email</p>
          <a href={SITE.emailHref} className="mt-1 block font-semibold break-all">
            {SITE.email}
          </a>
          <p className="mt-5 text-white/70 text-sm">Office Address</p>
          <address className="mt-1 not-italic text-sm leading-relaxed text-white/90">
            {SITE.name}<br />
            {SITE.address.line1}<br />
            {SITE.address.cityState}<br />
            {SITE.address.country}
          </address>
          <p className="mt-5 text-white/70 text-sm">Hours</p>
          <p className="mt-1 font-semibold">{SITE.hours}</p>
        </div>
      </div>

      <div className="relative border-t border-white/10">
        <div className="section py-6 text-xs text-white/55">
